...
------- qryInsertHFSLoansEOD    
---- Hua 20141010 Assume loanNumber is the unique ID in dbo_UV_HFSLoans_NOSHFD
-- Hua 20150312 let 06, 09 go through
SELECT 	hfs.LoanNumber,
		hfs.LoanInvestmentStatus,
		hfs.ChicagoParticipation,
		hfs.InterestRate,
...
WHERE hfs.LoanInvestmentStatus <> "03";


-------- qryAppendV2HFVloans ====== Hua 20150210
-- Hua 20150302 Added prefix F to Cusip
...
select LoanNumber, LOANLoanInvestmentStatus as LoanInvestmentStatus, ParticipationPercent AS ChicagoParticipation, InterestRate, 
	...
	("F" & ScheduleType & OriginationYear & AccountType & PassThruRate*100) AS CUSIP,
	Cdbl(0) AS Price
...
-- SELECT City, State, PFINumber, MANumber, DeliveryCommitmentNumber, LoanNumber, LoanRecordCreationDate, LastFundingEntryDate, 
-- 	ClosingDate, MPFBalance, LoanAmount, TransactionCode, InterestRate, ProductCode, NumberOfMonths, 
...
--------- qryselectallloans 
-- 20141210 Hua changed newCoupon, CouponOld and filter
-- Hua 20150305 Changed the newCoupon, CouponOld: use productCode, instead of loanInvestmetStatus, as test
SELECT City, State, PFINumber, MANumber, DeliveryCommitmentNumber, LoanNumber, LoanRecordCreationDate, LastFundingEntryDate, 
	ClosingDate, MPFBalance, LoanAmount, TransactionCode, InterestRate, ProductCode, NumberOfMonths, 
	IIF(Left([ProductCode],2) = "GL", IIF(Mid([ProductCode],3,1) = "0",(INT((InterestRate*200)-0.5)/200),CDbl(([InterestRate]) - 0.0046)),
		IIF([ProductCode] = "FX15", CDbl(([InterestRate]) - 0.0035),CDbl(([InterestRate]) - 0.0039))) AS NewCoupon, 
	IIF(Left([ProductCode],2) = "GL", 
		IIF(Mid([ProductCode],3,1) = "0", INT((InterestRate*200)-0.5)/200,CDbl(([InterestRate]) - ([ServicingFee] + [ExcessServicingFee]))), 
		CDbl(([InterestRate]) - 0.0039)) AS CouponOld, 
	CInt([InterestRate] * 800) / 800 AS PrepaymentInterestRate, FirstPaymentDate, MaturityDate, PIAmount, ScheduleCode, NonBusDayTrnEffDate, RemittanceTypeID, ExcessServicingFee, 
	ServicingFee, BatchID, ParticipationOrgKey, ParticipationPercent AS ChicagoParticipation, OrigLTV, LOANLoanInvestmentStatus
FROM dbo_UV_AllMPFLoans_NOSHFD AS mpf
...
-- and LOANLoanInvestmentStatus = "12"   ========= for V2 loans
...
SELECT m.city, m.state, m.pfinumber, m.manumber, m.deliverycommitmentnumber, m.loannumber, m.loanrecordcreationdate, m.lastfundingentrydate, m.closingdate, m.mpfbalance, 
	...
	IIf(LEFT(productCode,2)="GL" and MID(productCode,3,1)<>"0",IIf(ma.entryDate>=#2/1/2007#,m.newcoupon+0.0002,m.newcoupon),m.newcoupon) AS newcoupon2, m.couponold, 
	m.piamount, m.numberofmonths, m.schedulecode, m.nonbusdaytrneffdate, m.remittancetypeid, tblmonthenddate.monthenddate, m.excessservicingfee, m.servicingfee, 
	[cefee] + [ceperformance] AS macefee, ma.ceperformance AS ceperformancefee, m.batchid, m.chicagoparticipation, 
	m.OrigLTV, LOANLoanInvestmentStatus as LoanInvestmentStatus
FROM tblmonthenddate, tbltempmpf as m INNER JOIN tbltempma as ma ON (m.manumber = ma.manumber) AND (m.pfinumber = ma.pfinumber)
...
INSERT INTO tblmpfloansaggregatesource ( loannumber, deliverycommitmentnumber, manumber, pfinumber, loanrecordcreationdate, lastfundingentrydate, originalloanclosingdate, 
	...
	productcode, portfolioindicator, remittancetypeid, age, chicagoparticipation, currentloanbalance, entrydate, excessservicingfee, servicingfee, cefee, ceperformancefee, OrigLTV, LoanInvestmentStatus )
SELECT a.loannumber, a.deliverycommitmentnumber, a.manumber, a.pfinumber, a.loanrecordcreationdate, a.lastfundingentrydate, a.closingdate AS originalloanclosingdate, 
	...
	a.chicagoparticipation, NZ([sched end prin bal],[mpfbalance]) AS currentloanbalance, 
...
AND NZ([sched end prin bal],[mpfbalance]) > 0;
...
SELECT ProductCode, RemittanceTypeID, DeliveryYear, CStr(CInt([NoteRate]*200)/2) AS Wac, DeliveryMonth, 
	CInt(Sum(Abs([UnfundedAmountP])*[DeliveryDay])/Sum(Abs([UnfundedAmountP]))) AS WASettleDay, [Schedule Type], [Schedule Type], 
	Last(Delay) AS LastOfDelay, "MPFForward" AS Portfolio, CUSIP, CUSIP AS CUSIPHedged, 	
	IIF(Mid([ProductCode],3,2)="03",30,IIF(Mid([ProductCode],3,2)="05",15,Mid([ProductCode],3,2)))*12 AS Owam, 
	ProductCode AS [Sub Account Type], Wac AS [Account Type], CStr([DeliveryYear]) AS [Account Class], 
	...
	SUM(UnfundedAmountP*f.Coup)/SUM(UnfundedAmountP) AS Coup, 
	SUM(UnfundedAmountP*f.CoupOld)/SUM(UnfundedAmountP) AS CoupOld, 
	Coup AS PrepCoup, 
...
FROM [qryFundedandUnfundedDCsbyPFI Unhedged] as f
GROUP BY Agency, ProductCode, RemittanceTypeID, DeliveryYear, DeliveryMonth, [Schedule Type], CUSIP, CStr(CInt([NoteRate]*200)/2) 
...
-- - back up
-- SELECT ProductCode, IIf(Left([ProductCode],2)="GL",IIf(Right([ProductCode],2)="03" OR Right([ProductCode],2)="05","GNMA2","GNMA"),IIf(Left([ProductCode],2)="FX","FNMA"," ")) AS Agency, NoteRate, Fee, CDbl(0) AS Price, DeliveryStatus, ScheduleType, IIf(Left([ProductCode],2)="GL",1,dbo_UV_DCParticipation_MRA_NOSHFD.RemittanceTypeID) AS RemittanceTypeID, IIf(Left([ProductCode],2)="GL","GL",IIf([RemittanceTypeID]=1,"SS",IIf([RemittanceTypeID]=2,"AA",IIf([RemittanceTypeID]=3,"MA"," ")))) AS [Schedule Type], IIf([Schedule Type]="MS",18,IIf([Schedule Type]="GL",18,IIf([Schedule Type]="MA",2,IIf([Schedule Type]="AA",48,IIf([Schedule Type]="SS",18,18))))) AS Delay, EntryDate, EntryTime, DeliveryDate, 1 AS DTF, FullName, "1" AS PPMult, PFINumber, MANumber, DeliveryCommitmentNumber, DeliveryAmount, nz([DeliveryAmount])*nz([Participation]) AS DeliveryAmountP, FundedAmount, nz([FundedAmount])*nz([Participation]) AS FundedAmountP, ([DeliveryAmount])-([FundedAmount]) AS UnfundedAmount, [DeliveryAmountP]-[FundedAmountP] AS UnfundedAmountP, LastUpdatedDate, ScheduleCode, Participation, Year([DeliveryDate]) AS DeliveryYear, Month([DeliveryDate]) AS DeliveryMonth, Day([DeliveryDate]) AS DeliveryDay, [Schedule Type] & Right([DeliveryYear],4) & IIf([DeliveryMonth]<10,"0" & CStr([DeliveryMonth]),CStr([DeliveryMonth])) & Mid([ProductCode],3,2) & IIF(
-- 		Right([ProductCode],2) = "03" OR Right([ProductCode],2) = "05", INT(([NoteRate]*200)+0.5)/2, (CInt([NoteRate]*200)/2))*1000 AS CUSIP, IsExtended, ServicingFee, ExcessServicingFee, CEFee, CEPerformanceFee, IIf(Left([ProductCode],2)="GL",IIF(Right([ProductCode],2) = "03" OR Right([ProductCode],2) = "05", INT((NoteRate*200)-0.5)/200, CDbl([NoteRate])-0.0046),
-- 		IIf([ProductCode]="FX15",CDbl([NoteRate])-0.0035,CDbl([NoteRate])-0.0039)) AS Coup, IIf(Left([ProductCode],2)="GL",IIF(Right([ProductCode],2) = "03" OR Right([ProductCode],2) = "05", INT((NoteRate*200)-0.5)/200,
-- 		CDbl([NoteRate])-([ServicingFee]+[ExcessServicingFee]+[CEFee])),CDbl([NoteRate])-0.0039) AS CoupOld INTO ForwardSettleDCs
-- FROM dbo_UV_DCParticipation_MRA_NOSHFD
-- WHERE ParticipationOrgKey = 3
-- ORDER BY NoteRate, DeliveryDate;
...
SELECT "MPF" AS Portfolio, 
	...
	CInt(NPer(([a.InterestRate]/12),([a.PIAmount]*-1),[a.CurrentLoanBalance])) AS WAM, Age,
...
-- _DlyMPFDC ========== Hua 20150305 changed subActII to GN, GL, FX
SELECT 'DlyMPFDC' AS Query, "Asset MPFDC " & SubActII AS Account, 'MBS' AS [Sec Type], 'Asset' AS BSAccount, 'MPFDC' AS SubActI, 
	iif(right(ProductCode,2) IN ("03","05"),"GN",Left(ProductCode,2)) AS SubActII, 
	CUSIP AS [CUST ID], CUSIP, Owam AS [Orig Term], 
	AggNotional AS Holding, Coup AS Coupon, Wac AS [WAC(coll)], 
	Wam AS [WAM(coll)], Age AS [WALA(coll)], 1 AS Factor, P AS Price, 
	Agency, 'N' AS [Use Static Model], 'USER' AS [Settlement Type], Settle AS [Settle Date], '30/360' AS DayCount, 
...
-- back up DlyMPF
-- SELECT 'DlyMPF' AS Query, "Asset MPF " & IIF(tblSecuritiesPalmsSource.AccountType = "03" Or tblSecuritiesPalmsSource.AccountType = "05", "Govt Loans", Left([CUSIP],2)) AS Account, 'MBS' AS [Sec Type], 'Asset' AS BSAccount, 'MPF' AS SubActI, IIF(tblSecuritiesPalmsSource.AccountType = "03" Or tblSecuritiesPalmsSource.AccountType = "05", "Govt Loans", Left([CUSIP],2)) AS SubActII, tblSecuritiesPalmsSource.CUSIP AS [CUST ID], tblSecuritiesPalmsSource.CUSIP AS CUSIP, tblSecuritiesPalmsSource.AggOWAM AS [Orig Term], [AggBookPrice]-IIf([month 1] Is Null,0,[Month 1]) AS Holding, tblSecuritiesPalmsSource.AggCoup AS Coupon, tblSecuritiesPalmsSource.AggWac AS [WAC(coll)], [AggWam]-IIf([WAM_adj] Is Null,0,[WAM_Adj]) AS [WAM(coll)], tblSecuritiesPalmsSource.AggAge AS [WALA(coll)], tblSecuritiesPalmsSource.Delay, 1 AS Factor, tblSecuritiesPalmsSource.AggPrice AS Price, tblSecuritiesPalmsSource.Agency, 'T+0' AS [Settlement Type], 'N' AS [Use Static Model], '30/360' AS DayCount, 'USER' AS Source, 'Price' AS PriceAnchor, 'tblSecuritiesPalmsSource in N:\Palms\BankDB\MPFFIVES for MiddlewareSQLandNewRemit.mdb' AS PriceSource, qryWALoanSize_MPF.WALoanSize AS WALoanSize, qryWALoanSize_MPF.LTV
-- FROM (tblSecuritiesPalmsSource LEFT JOIN MRA_MPFPayDown ON tblSecuritiesPalmsSource.CUSIP=MRA_MPFPayDown.CUSTOM2) LEFT JOIN qryWALoanSize_MPF ON tblSecuritiesPalmsSource.CUSIP=qryWALoanSize_MPF.CUSIP;
-- 
...
SELECT 'DlyMPF' AS Query, "Asset MPF " & SubActII AS Account, 'MBS' AS [Sec Type], 'Asset' AS BSAccount, 'MPF' AS SubActI, 
	...
	IIf(ProductCode in ("GL03", "GL05"), IIf(RIGHT(ProductCode,1)="3","GN30","GN15"), ProductCode)) AS SubActII, 
...
-- 	IIf(Left([ProductCode],2)="MS","MS",IIf(Left([ProductCode],2)="GL","GL",IIf([RemittanceTypeID]=1,"SS",IIf([RemittanceTypeID]=3,"MA","AA")))) AS ScheduleType, 
-- 	IIf([PortfolioIndicator]="BATCH",Year([OriginalLoanClosingDate]),Year([LoanRecordCreationDate])) AS OriginationYear, 
-- 	Right([ProductCode],2) AS AccountType, (CInt([InterestRate]*200)/200)*100 AS PassThruRate, OriginationYear AS AccountClass, 
...
SELECT MAS.LoanNumber, 
	...
	IIf([PortfolioIndicator]="BATCH",Year([OriginalLoanClosingDate]),Year([LoanRecordCreationDate])) AS OriginationYear, 
	Right([ProductCode],2) AS AccountType, (CInt([InterestRate]*200)/200)*100 AS PassThruRate, OriginationYear AS AccountClass, 
	IIf(LoanInvestmentStatus="11","C",	IIf(LoanInvestmentStatus="12","F","")) &
	IIf(SWAPHedgeID Is Null,[ScheduleType] & [OriginationYear] & [AccountType] & ([PassThruRate]*100),
		"DC" & Right(SwapHedgeID,2) & ScheduleType & OriginationYear & AccountType & PassThruRate) AS CUSIP, 
...
SELECT 
   ...
   (CInt([InterestRate]*200)/200)*100 AS PassThruRate, 
	...
	IIF (h.SwapHedgeID IS NULL,
		IIf(LoanInvestmentStatus="11","C",
		IIf(LoanInvestmentStatus="12","F","")) & ScheduleType & OriginationYear & AccountType & (PassThruRate*100), 
      "DC" & Right([SwapHedgeID], 2) & [ScheduleType] & Right([OriginationYear],4) & [AccountType] & [PassThruRate]) AS Cusip,
