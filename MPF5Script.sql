...
------- qryDeleteMPFandForwardsfromInstrumenttblProduction --- Command59_Click()
...
WHERE (((qryDate)=CDate([ForwardSettleDCs].[DeliveryDate])) AND ((CStr([NoteRate]))=[qryMPFForwardPrice].[Rate]) 
AND ((qryRate)=CStr([ForwardSettleDCs].[NoteRate])) AND ((ForwardSettleDCs.[Schedule Type])=[ProdType]) AND ((ForwardSettleDCs.ProductCode)=[CatType]));
...
-- DlyMPF
SELECT 'DlyMPF' AS Query, "Asset MPF " & Left([CUSIP],2) AS Account, 'MBS' AS [Sec Type], 'Asset' AS BSAccount, 'MPF' AS SubActI, Left([CUSIP],2) AS SubActII, 
	...
	[AggBookPrice]-IIf([month 1] Is Null,0,[Month 1]) AS Holding, tblSecuritiesPalmsSource.AggCoup AS Coupon, tblSecuritiesPalmsSource.AggWac AS [WAC(coll)], 
	[AggWam]-IIf([WAM_adj] Is Null,0,[WAM_Adj]) AS [WAM(coll)], tblSecuritiesPalmsSource.AggAge AS [WALA(coll)], tblSecuritiesPalmsSource.Delay, 1 AS Factor, 
	tblSecuritiesPalmsSource.AggPrice AS Price, tblSecuritiesPalmsSource.Agency, 'T+0' AS [Settlement Type], 'N' AS [Use Static Model], '30/360' AS DayCount, 
	'USER' AS Source, 'Price' AS PriceAnchor, 'tblSecuritiesPalmsSource in N:\Palms\BankDB\MPFFIVES for MiddlewareSQLandNewRemit.mdb' AS PriceSource, 
	qryWALoanSize_WALoanSize AS WALoanSize, qryWALoanSize_LTV
FROM (tblSecuritiesPalmsSource LEFT JOIN MRA_MPFPayDown ON tblSecuritiesPalmsSource.CUSIP=MRA_MPFPayDown.CUSTOM2) 
	LEFT JOIN qryWALoanSize_MPF ON tblSecuritiesPalmsSource.CUSIP=qryWALoanSize_CUSIP;
...
-- DlyMPFDC
SELECT Query, Account, [Sec Type], BSAccount, SubActI, SubActII, [CUST ID], [_DlyMPFDC].CUSIP, [Orig Term], Holding, Coupon, [WAC(coll)], [WAM(coll)], [WALA(coll)], 
...
SELECT qryPalmsDataSource.AccountClass, qryPalmsDataSource.AccountType, qryPalmsDataSource.ScheduleType, qryPalmsDataSource.PassThruRate, Last(qryPalmsDataSource.Portfolio) AS Portfolio, Last(qryPalmsDataSource.CUSIP) AS CUSIP, Count(qryPalmsDataSource.LoanNumber) AS H2, Sum(qryPalmsDataSource.Notional) AS AggNotional, Last(qryPalmsDataSource.Mult) AS Mult, IIf(Sum([CurrentLoanBalance])=0,Sum([OriginalAmount])/Sum([OriginalAmount]),Sum([CurrentLoanBalance])/Sum([OriginalAmount])) AS AggFactor, Last(qryPalmsDataSource.[Add Accrued?]) AS [Add Accrued?], Last(qryPalmsDataSource.PV) AS PV, Last(qryPalmsDataSource.Swap) AS Swap, (Sum([Wac]*[AcctBalance])/Sum([AcctBalance])) AS AggWac, (Sum([Coup]*[AcctBalance])/Sum([AcctBalance])) AS AggCoup, CInt(Sum([Wam]*[AcctBalance])/Sum([AcctBalance])) AS AggWam, (Sum([Age]*[AcctBalance])/Sum([AcctBalance])) AS AggAge, IIf([AccountType]=30 And [AggWam]<340,"Seasoned",IIf([AccountType]=20 And [AggWam]<200,"Seasoned",IIf([AccountType]=15 And [AggWam]<160,"Seasoned","MPF"))) AS H1, Last(qryPalmsDataSource.Swam) AS Swam, CInt(Sum([OWAM]*[AcctBalance])/Sum([AcctBalance])) AS AggOWAM, "OAS" AS [P/O], 0 AS AggPrice, 0 AS OAS, Last(qryPalmsDataSource.Settle) AS Settle, Last(qryPalmsDataSource.Repo) AS Repo, Last(qryPalmsDataSource.NxtPmt) AS NxtPmt, "PA" AS PP, Last(qryPalmsDataSource.PPConst) AS PPConst, Last(qryPalmsDataSource.PPMult) AS PPMult, Last(qryPalmsDataSource.PPSense) AS PPSense, Last(qryPalmsDataSource.Lag) AS Lag, Last(qryPalmsDataSource.Lock) AS Lock, Last(qryPalmsDataSource.PPFq) AS PPFq, Last(qryPalmsDataSource.NxtPP) AS NxtPP, Last(qryPalmsDataSource.NxtRst1) AS NxtRst1, (Sum([PrepWac]*[AcctBalance])/Sum([AcctBalance])) AS AggPrepWac, (Sum([PrepCoup]*[AcctBalance])/Sum([AcctBalance])) AS AggPrepCoup, Last(qryPalmsDataSource.FA) AS FA, Last(qryPalmsDataSource.Const1) AS Const1, Last(qryPalmsDataSource.Const2) AS Const2, Last(qryPalmsDataSource.Rate) AS Rate, Last(qryPalmsDataSource.RF) AS RF, Last(qryPalmsDataSource.Floor) AS Floor, Last(qryPalmsDataSource.Cap) AS Cap, Last(qryPalmsDataSource.PF) AS PF, Last(qryPalmsDataSource.PF1) AS PF1, Last(qryPalmsDataSource.PF2) AS PF2, Last(qryPalmsDataSource.PC) AS PC, Last(qryPalmsDataSource.AB) AS AB, Last(qryPalmsDataSource.NxtRst2) AS NxtRst2, Last(qryPalmsDataSource.LookBackRate) AS LookBackRate1, Last(qryPalmsDataSource.LookBack2) AS LookBack2, Last(qryPalmsDataSource.WamDate) AS WamDate, Last(qryPalmsDataSource.PPUnits) AS PPUnits, Last(qryPalmsDataSource.PPCRShift) AS PPCRShift, Last(qryPalmsDataSource.RcvCurrEscr) AS RcvCurrEscr, Last(qryPalmsDataSource.PayCurrEscr) AS PayCurrEscr, Sum([CurrentLoanBalance]*[ChicagoParticipation]) AS AggBookPrice, Last(qryPalmsDataSource.AmortMethod) AS AmortMethod, Last(qryPalmsDataSource.ClientName) AS ClientName, Last(qryPalmsDataSource.[Active?]) AS [Active?], Last(qryPalmsDataSource.Agency) AS Agency, Last(qryPalmsDataSource.[Int Coup Mult]) AS [Int Coup Mult], Last(qryPalmsDataSource.[PP Coup Mult]) AS [PP Coup Mult], Last(qryPalmsDataSource.[Sum Floor]) AS [Sum Floor], Last(qryPalmsDataSource.[Sum Cap]) AS [Sum Cap], Last(qryPalmsDataSource.[Servicing Model]) AS [Servicing Model], Last(qryPalmsDataSource.[Loss Model]) AS [Loss Model], Last(qryPalmsDataSource.[Sched Cap?]) AS [Sched Cap?], Last(qryPalmsDataSource.Delay) AS Delay, Last(qryPalmsDataSource.Ballon) AS Ballon, Last(qryPalmsDataSource.IF) AS IF, Last(qryPalmsDataSource.Mult1) AS Mult1, Last(qryPalmsDataSource.Mult2) AS Mult2, (Sum([PrepaymentInterestRate]*[AcctBalance])/Sum([AcctBalance])) AS AggPrepaymentInterestRate
FROM qryPalmsDataSource
GROUP BY qryPalmsDataSource.AccountClass, qryPalmsDataSource.AccountType, qryPalmsDataSource.ScheduleType, qryPalmsDataSource.PassThruRate, qryPalmsDataSource.OriginationYear
HAVING ((((Sum([Age]*[AcctBalance])/Sum([AcctBalance])))>30))
ORDER BY qryPalmsDataSource.OriginationYear, qryPalmsDataSource.AccountType DESC , qryPalmsDataSource.ScheduleType DESC , (Sum([PrepaymentInterestRate]*[AcctBalance])/Sum([AcctBalance]));



-- qryAggregateForwardSettleCommitmentsdaily
INSERT INTO tblForwardCommitmentPalmsSourcedaily ( ProductCode, RemittanceTypeID, DeliveryYear, Wac, DeliveryMonth, WASettleDay, ScheduleType, NxtPmtBase, Portfolio, CUSIP, Owam, [Sub Account Type], [Account Type], [Account Class], H1, H2, Notional, Mult, Factor, [Add Accrued?], PV, Swap, Coup, Age, Wam, Swam, [P/O], OAS, P, Settle, Repo, NxtPmt, PP, PPConst, PPMult, PPSens, Lag, Lock, PPFq, NxtPP, NxtRst1, PrepWac, PrepCoup, FA1, Const1, Mult1, Rate1, RF1, Floor1, Cap1, PF1, PC1, AB1, NxtRst2, BOOK, ClientName, WAPrice, NumberofCommitments, Agency, [Entry Date] )
SELECT qryFundedandUnfundedDCsbyPFIdaily.ProductCode, qryFundedandUnfundedDCsbyPFIdaily.RemittanceTypeID, qryFundedandUnfundedDCsbyPFIdaily.DeliveryYear, CStr((CInt([NoteRate]*200)/200)*100) AS Wac, qryFundedandUnfundedDCsbyPFIdaily.DeliveryMonth, CInt(Sum([UnfundedAmountP]*[DeliveryDay])/Sum([UnfundedAmountP])) AS WASettleDay, IIf(Left([ProductCode],2)="GL","GL",IIf([RemittanceTypeID]=1,"SS",IIf([RemittanceTypeID]=2,"AA"," "))) AS [Schedule Type], IIf([WASettleDay]<18,DateSerial(Year(CDate([Settle])),Month(CDate([Settle]))+1,Day(CDate([Settle]))),DateSerial(Year(CDate([Settle])),Month(CDate([Settle]))+2,Day(CDate([Settle])))) AS NxtPmtBase, "Hedging" AS Portfolio, [Schedule Type] & Right([DeliveryYear],4) & IIf([DeliveryMonth]<10,"0" & CStr([DeliveryMonth]),CStr([DeliveryMonth])) & Mid([ProductCode],3,2) & [WAC]*1000 & "FH" AS CUSIP, Mid([ProductCode],3,2)*12 AS Owam, qryFundedandUnfundedDCsbyPFIdaily.ProductCode AS [Sub Account Type], CStr((CInt([NoteRate]*200)/200)*100) AS [Account Type], CStr([DeliveryYear]) AS [Account Class], CStr([WAPrice]) AS H1, CStr([WASettleDay]) AS H2, CStr(Sum([UnfundedAmountP])) AS Notional, "1" AS Mult, "1" AS Factor, "0" AS [Add Accrued?], "Mid" AS PV, "Mid" AS Swap, CStr(CDbl([Wac])-(Last([ServicingFee])+Last([CEFee])+Last([ExcessServicingFee]))*100) AS Coup, "0" AS Age, [OWAM] AS Wam, IIf([OWAM]=180,160,IIf([OWAM]=360,335,IIf([OWAM]=240,220,""))) AS Swam, "OAS" AS [P/O], 0 AS OAS, (Sum([Price]*[UnfundedAmountP]))/(Sum([UnfundedAmountP])) AS P, IIf([DeliveryMonth]<10,"0" & CStr([DeliveryMonth]),CStr([DeliveryMonth])) & "/" & IIf([WASettleDay]<10,"0" & CStr([WASettleDay]),CStr([WASettleDay])) & "/" & CStr([DeliveryYear]) AS Settle, DLookUp("[RepoRate]","[tblPALMSRepo]") AS Repo, IIf(Month([NxtPmtBase])<10,"0" & CStr(Month([NxtPmtBase])),CStr(Month([NxtPmtBase]))) & "/18/" & CStr(Year([NxtPmtBase])) AS NxtPmt, "PA" AS PP, "0" AS PPConst, "1" AS PPMult, "1" AS PPSens, "0" AS Lag, "0" AS Lock, "12" AS PPFq, IIf(Month([NxtPmtBase])<10,"0" & CStr(Month([NxtPmtBase])),CStr(Month([NxtPmtBase]))) & "/18/" & CStr(Year([NxtPmtBase])) AS NxtPP, IIf(Month([NxtPmtBase])<10,"0" & CStr(Month([NxtPmtBase])),CStr(Month([NxtPmtBase]))) & "/18/" & CStr(Year([NxtPmtBase])) AS NxtRst1, [Wac] AS PrepWac, [Coup] AS PrepCoup, "F" AS FA1, "0" AS Const1, "0" AS Mult1, "3L" AS Rate1, "12" AS RF1, "None" AS Floor1, "None" AS Cap1, "None" AS PF1, "None" AS PC1, "Bond" AS AB1, [NxtRst1] AS NxtRst2, CStr(Sum([UnfundedAmountP])) AS BOOK, "Hedging" AS ClientName, 0 AS WAPrice, Count(qryFundedandUnfundedDCsbyPFIdaily.NoteRate) AS NumberofCommitments, qryFundedandUnfundedDCsbyPFIdaily.Agency, qryFundedandUnfundedDCsbyPFIdaily.EntryDate
FROM qryFundedandUnfundedDCsbyPFIdaily
GROUP BY qryFundedandUnfundedDCsbyPFIdaily.RemittanceTypeID, qryFundedandUnfundedDCsbyPFIdaily.DeliveryYear, qryFundedandUnfundedDCsbyPFIdaily.DeliveryMonth, qryFundedandUnfundedDCsbyPFIdaily.ProductCode, CStr((CInt([NoteRate]*200)/200)*100), 0, qryFundedandUnfundedDCsbyPFIdaily.Agency, qryFundedandUnfundedDCsbyPFIdaily.EntryDate, qryFundedandUnfundedDCsbyPFIdaily.Agency
HAVING (((CStr(Sum([UnfundedAmountP])))>0));
...
SELECT [qryFundedandUnfundedDCsbyPFI Hedged].ProductCode, [qryFundedandUnfundedDCsbyPFI Hedged].RemittanceTypeID, [qryFundedandUnfundedDCsbyPFI Hedged].DeliveryYear, CStr((CInt([NoteRate]*200)/200)*100) AS Wac, IIf(Left(Last([qryFundedandUnfundedDCsbyPFI Hedged].[ProductCode]),2)="GL",CStr(CDbl([Wac])-0.46),IIf([qryFundedandUnfundedDCsbyPFI Hedged].[ProductCode]="FX15",CStr(CDbl([Wac])-0.35),CStr(CDbl([Wac])-0.39))) AS Coup, IIf(Left(Last([qryFundedandUnfundedDCsbyPFI Hedged].[ProductCode]),2)="GL",CStr(CDbl([Wac])-(Last([ServicingFee])+Last([CEFee])+Last([ExcessServicingFee]))*100),CStr(CDbl([Wac])-0.39)) AS CoupOld, [qryFundedandUnfundedDCsbyPFI Hedged].DeliveryMonth, CInt(Sum(Abs([UnfundedAmountP])*[DeliveryDay])/Sum(Abs([UnfundedAmountP]))) AS WASettleDay, [qryFundedandUnfundedDCsbyPFI Hedged].[Schedule Type], [qryFundedandUnfundedDCsbyPFI Hedged].[Schedule Type], Last([qryFundedandUnfundedDCsbyPFI Hedged].Delay) AS LastOfDelay, "MPFForward" AS Portfolio, Last(("DC" & [owam] & [Account Type])) AS CUSIP, "DC" & IIf([SwapHedgeID] Is Null,"00",Right([SwapHedgeID],2)) & [Schedule Type] & Right([DeliveryYear],4) & Mid([ProductCode],3,2) & [WAC]*1000 AS CUSIPHedged, Mid([ProductCode],3,2)*12 AS Owam, [qryFundedandUnfundedDCsbyPFI Hedged].ProductCode AS [Sub Account Type], CStr((CInt([NoteRate]*200)/200)*100) AS [Account Type], CStr([DeliveryYear]) AS [Account Class], [Schedule Type] & Right([DeliveryYear],4) & IIf([DeliveryMonth]<10,"0" & CStr([DeliveryMonth]),CStr([DeliveryMonth])) & Mid([ProductCode],3,2) & [WAC]*1000 AS H1, CStr([WASettleDay]) AS H2, CStr(Sum([UnfundedAmountP])) AS Notional, "1" AS Mult, "1" AS Factor, "1" AS [Add Accrued?], "Mid" AS PV, "Mid" AS Swap, "1" AS Age, [OWAM]-1 AS Wam, IIf([OWAM]=180,160,IIf([OWAM]=360,335,IIf([OWAM]=240,220,""))) AS Swam, "Price" AS [P/O], 0 AS OAS, Sum([deliveryamount]*[price])/Sum([deliveryamount]) AS P, IIf([DeliveryMonth]<10,"0" & CStr([DeliveryMonth]),CStr([DeliveryMonth])) & "/" & IIf([WASettleDay]<10,"0" & CStr([WASettleDay]),CStr([WASettleDay])) & "/" & CStr([DeliveryYear]) AS Settle, DLookUp("[RepoRate]","[tblPALMSRepo]") AS Repo, NextDateFwd([DeliveryYear],[DeliveryMonth],[WASettleDay],[Schedule Type]) AS NxtPmt, "AFT" AS PP, "0" AS PPConst, Last([qryFundedandUnfundedDCsbyPFI Hedged].PPMult) AS LastOfPPMult, "1" AS PPSens, "0" AS Lag, "0" AS Lock, 0 AS PPCRShift, "12" AS PPFq, [NxtPmt] AS NxtPP, [NxtPmt] AS NxtRst1, [Wac] AS PrepWac, IIf(Left(Last([qryFundedandUnfundedDCsbyPFI Hedged].[ProductCode]),2)="GL",CStr(CDbl([Wac])-0.46),IIf([qryFundedandUnfundedDCsbyPFI Hedged].[ProductCode]="FX15",CStr(CDbl([Wac])-0.35),CStr(CDbl([Wac])-0.39))) AS PrepCoup, "F" AS FA1, "0" AS Const1, "0" AS Mult1, "3L" AS Rate1, "12" AS RF1, "None" AS Floor1, "None" AS Cap1, "None" AS PF1, "None" AS PC1, "Bond" AS AB1, [NxtRst1] AS NxtRst2, CStr(Sum([UnfundedAmountP])) AS BOOK, Last([qryFundedandUnfundedDCsbyPFI Hedged].HedgeID) AS ClientName, Sum([qryFundedandUnfundedDCsbyPFI Hedged].UnfundedAmountP) AS AggNotional, 0 AS WAPrice, Count([qryFundedandUnfundedDCsbyPFI Hedged].NoteRate) AS NumberofCommitments, [qryFundedandUnfundedDCsbyPFI Hedged].Agency
FROM [qryFundedandUnfundedDCsbyPFI Hedged]
GROUP BY [qryFundedandUnfundedDCsbyPFI Hedged].RemittanceTypeID, [qryFundedandUnfundedDCsbyPFI Hedged].DeliveryYear, [qryFundedandUnfundedDCsbyPFI Hedged].DeliveryMonth, [qryFundedandUnfundedDCsbyPFI Hedged].[Schedule Type], [qryFundedandUnfundedDCsbyPFI Hedged].ProductCode, CStr((CInt([NoteRate]*200)/200)*100), [qryFundedandUnfundedDCsbyPFI Hedged].Agency, [qryFundedandUnfundedDCsbyPFI Hedged].[Schedule Type], [qryFundedandUnfundedDCsbyPFI Hedged].PurchaseGroup, [qryFundedandUnfundedDCsbyPFI Hedged].Agency, [qryFundedandUnfundedDCsbyPFI Hedged].SwapHedgeID
HAVING (((CStr(Sum([UnfundedAmountP])))>0));
...
-- qryAggregateHedgedForwardSettleMarkToModelCommitments
INSERT INTO tblForwardCommitmentMarkToModel ( ProductCode, RemittanceTypeID, DeliveryYear, Wac, DeliveryMonth, WASettleDay, ScheduleType, ScheduleType2, Delay, Portfolio, CUSIP, CusipHedged, Owam, [Sub Account Type], [Account Type], [Account Class], H1, H2, Notional, Mult, Factor, [Add Accrued?], PV, Swap, Age, Wam, Swam, [P/O], OAS, P, Settle, Repo, NxtPmt, PP, PPConst, PPMult, PPSens, Lag, Lock, PPCRShift, PPFq, NxtPP, NxtRst1, PrepWac, Coup, CoupOld, PrepCoup, FA1, Const1, Mult1, Rate1, RF1, Floor1, Cap1, PF1, PC1, AB1, NxtRst2, BOOK, ClientName, AggNotional, WAPrice, NumberofCommitments, Agency )
SELECT [qryFundedandUnfundedDCsbyPFI Hedged].ProductCode, [qryFundedandUnfundedDCsbyPFI Hedged].RemittanceTypeID, [qryFundedandUnfundedDCsbyPFI Hedged].DeliveryYear, CStr((CInt([NoteRate]*200)/200)*100) AS Wac, [qryFundedandUnfundedDCsbyPFI Hedged].DeliveryMonth, CInt(Sum([UnfundedAmountP]*[DeliveryDay])/Sum([UnfundedAmountP])) AS WASettleDay, [qryFundedandUnfundedDCsbyPFI Hedged].[Schedule Type], [qryFundedandUnfundedDCsbyPFI Hedged].[Schedule Type], Last([qryFundedandUnfundedDCsbyPFI Hedged].Delay) AS LastOfDelay, "MPFFwdAct" AS Portfolio, Last(("DC" & [owam] & [Account Type])) AS CUSIP, ("HC" & IIF([SwapHedgeID] is Null, "00", Right([SwapHedgeID], 2)) & [Schedule Type] & Right([DeliveryYear],4) & Mid([ProductCode],3,2) & ([WAC]*1000) & "A") AS CUSIPHedged, Mid([ProductCode],3,2)*12 AS Owam, [qryFundedandUnfundedDCsbyPFI Hedged].ProductCode AS [Sub Account Type], CStr((CInt([NoteRate]*200)/200)*100) AS [Account Type], CStr([DeliveryYear]) AS [Account Class], [Schedule Type] & Right([DeliveryYear],4) & IIf([DeliveryMonth]<10,"0" & CStr([DeliveryMonth]),CStr([DeliveryMonth])) & Mid([ProductCode],3,2) & [WAC]*1000 AS H1, CStr([WASettleDay]) AS H2, CStr(Sum([UnfundedAmountP])) AS Notional, "1" AS Mult, "1" AS Factor, "1" AS [Add Accrued?], "Mid" AS PV, "Mid" AS Swap, "0" AS Age, [OWAM] AS Wam, IIf([OWAM]=180,160,IIf([OWAM]=360,335,IIf([OWAM]=240,220,""))) AS Swam, "Price" AS [P/O], 0 AS OAS, 0 AS P, IIf([DeliveryMonth]<10,"0" & CStr([DeliveryMonth]),CStr([DeliveryMonth])) & "/" & IIf([WASettleDay]<10,"0" & CStr([WASettleDay]),CStr([WASettleDay])) & "/" & CStr([DeliveryYear]) AS Settle, DLookUp("[RepoRate]","[tblPALMSRepo]") AS Repo, NextDateFwd([DeliveryYear],[DeliveryMonth],[WASettleDay],[Schedule Type]) AS NxtPmt, "AFT" AS PP, "0" AS PPConst, Last([qryFundedandUnfundedDCsbyPFI Hedged].PPMult) AS LastOfPPMult, "1" AS PPSens, "0" AS Lag, "0" AS Lock, 0 AS PPCRShift, "12" AS PPFq, [NxtPmt] AS NxtPP, [NxtPmt] AS NxtRst1, [Wac] AS PrepWac, IIf(Left(Last([qryFundedandUnfundedDCsbyPFI Hedged].[ProductCode]),2)="GL",CStr(CDbl([Wac])-0.46),IIf([qryFundedandUnfundedDCsbyPFI Hedged].[ProductCode]="FX15",CStr(CDbl([Wac])-0.35),CStr(CDbl([Wac])-0.39))) AS Coup, IIf(Left(Last([qryFundedandUnfundedDCsbyPFI Hedged].[ProductCode]),2)="GL",CStr(CDbl([Wac])-(Last([ServicingFee])+Last([CEFee])+Last([ExcessServicingFee]))*100),CStr(CDbl([Wac])-0.39)) AS CoupOld, IIf(Left(Last([qryFundedandUnfundedDCsbyPFI Hedged].[ProductCode]),2)="GL",CStr(CDbl([Wac])-0.46),IIf([qryFundedandUnfundedDCsbyPFI Hedged].[ProductCode]="FX15",CStr(CDbl([Wac])-0.35),CStr(CDbl([Wac])-0.39))) AS PrepCoup, "F" AS FA1, "0" AS Const1, "0" AS Mult1, "3L" AS Rate1, "12" AS RF1, "None" AS Floor1, "None" AS Cap1, "None" AS PF1, "None" AS PC1, "Bond" AS AB1, [NxtRst1] AS NxtRst2, CStr(Sum([UnfundedAmountP])) AS BOOK, Last([qryFundedandUnfundedDCsbyPFI Hedged].HedgeID) AS ClientName, Sum([qryFundedandUnfundedDCsbyPFI Hedged].UnfundedAmountP) AS AggNotional, 0 AS WAPrice, Count([qryFundedandUnfundedDCsbyPFI Hedged].NoteRate) AS NumberofCommitments, [qryFundedandUnfundedDCsbyPFI Hedged].Agency
FROM [qryFundedandUnfundedDCsbyPFI Hedged]
GROUP BY [qryFundedandUnfundedDCsbyPFI Hedged].ProductCode, [qryFundedandUnfundedDCsbyPFI Hedged].RemittanceTypeID, [qryFundedandUnfundedDCsbyPFI Hedged].DeliveryYear, CStr((CInt([NoteRate]*200)/200)*100), [qryFundedandUnfundedDCsbyPFI Hedged].DeliveryMonth, [qryFundedandUnfundedDCsbyPFI Hedged].[Schedule Type], [qryFundedandUnfundedDCsbyPFI Hedged].[Schedule Type], [qryFundedandUnfundedDCsbyPFI Hedged].Agency, [qryFundedandUnfundedDCsbyPFI Hedged].PurchaseGroup, [qryFundedandUnfundedDCsbyPFI Hedged].Agency, [qryFundedandUnfundedDCsbyPFI Hedged].SwapHedgeID
HAVING (((CStr(Sum([UnfundedAmountP])))>0));

-- qryAppendAllBatchLoansFromWarehouse -------- No use
INSERT INTO tblTempMPFBatch ( 
	City, State, PFINumber, MANumber, DeliveryCommitmentNumber, LoanNumber, LoanRecordCreationDate, LastFundingEntryDate, ClosingDate, MPFBalance, LoanAmount, TransactionCode, 
	InterestRate, ProductCode, NumberOfMonths, NewCoupon, CouponOld, PrepaymentInterestRate, FirstPaymentDate, MaturityDate, PIAmount, ScheduleCode, NonBusDayTrnEffDate, 
	RemittanceTypeID, ExcessServicingFee, ServicingFee )
SELECT DWView.City, DWView.State, DWView.PFINumber, DWView.MANumber, DWView.DeliveryCommitmentNumber, DWView.LoanNumber, [DWView]![EntryDate] AS LoanRecordCreationDate,
	[DWView]![EntryDate] AS LastFundingEntryDate, DWView.LoanClosingDate AS ClosingDate, [tblLoanFundingSumView]![Amount] AS MPFBalance, DWView.LoanAmount, 
	DWView.TransactionCode, DWView.InterestRate, DWView.ProductCode, DWView.NumberOfMonths, 
	IIf(Left([DWView].[ProductCode],2)="GL",CDbl([DWView]![InterestRate])-0.0046,IIf([DWView].[ProductCode]="FX15",CDbl([DWView]![InterestRate])-0.0035,CDbl([DWView]![InterestRate])-0.0039)) AS NewCoupon, 
	IIf(Left([DWView].[ProductCode],2)="GL",CDbl([DWView]![InterestRate])-([DWView]![ServicingFee]+[DWView]![ExcessServicingFee]),CDbl([DWView]![InterestRate])-0.0039) AS CouponOld, 
	CInt([DWView]![InterestRate]*800)/800 AS PrepaymentInterestRate, DWView.FirstPaymentDate, DWView.MaturityDate, DWView.PIAmount, DWView.ScheduleCode, DWView.NonBusDayTrnEffDate, 
	DWView.RemittanceTypeID, DWView.ExcessServicingFee, DWView.ServicingFee
FROM ((((((tblLoan 
	INNER JOIN (tblLoanFunding INNER JOIN tblDeliveryCommitment ON tblLoanFunding.DeliveryCommitmentNumber = tblDeliveryCommitment.DeliveryCommitmentNumber) 
		ON tblLoan.LoanNumber = tblLoanFunding.LoanNumber) 
	INNER JOIN tblSchedule ON tblDeliveryCommitment.ScheduleCode = tblSchedule.ScheduleCode) 
	INNER JOIN tblPFI ON tblLoanFunding.PFINumber = tblPFI.PFINumber) 
	INNER JOIN tblLoanFundingSumView ON tblLoanFunding.LoanNumber = tblLoanFundingSumView.LoanNumber) 
	INNER JOIN tblPFILoan ON (tblLoan.PFINumber = tblPFILoan.PFINumber) AND (tblLoan.PFILoanNumber = tblPFILoan.PFILoanNumber)) 
	INNER JOIN tblLoanFundingDateViewTABLE 
		ON (tblLoanFunding.PFINumber = tblLoanFundingDateViewTABLE.PFINumber) AND (tblLoanFunding.LoanFundingNumber = tblLoanFundingDateViewTABLE.LoanFundingNumber)) 
	INNER JOIN dbo_UV_AllLoansToMPFLoansAggegateSourceOne_NOSHFD AS DWView ON tblLoan.LoanNumber = DWView.LoanNumber
WHERE (((DWView.LoanNumber)=1111694) AND ((DWView.BatchID) Is Not Null));
...
SELECT hfs.LoanNumber, hfs.LoanInvestmentStatus, hfs.ChicagoParticipation, hfs.InterestRate, hfs.FundingDate, hfs.ProductCode, hfs.Action_Code, LEFT(hfs.ProductCode,2) AS ScheduleType, CINT(((hfs.InterestRate*20000)-50)/100)/2 AS PassThruRate, Right(hfs.ProductCode,2) AS AccountType, NZ(Year(mpf.LoanRecordCreationDate),Year(Date())) AS OriginationYear, (ScheduleType & OriginationYear & AccountType & PassThruRate*100) AS CUSIP, Cdbl(0) AS Price INTO tblHFSLoansEOD
FROM (dbo_UV_HFSLoans_NOSHFD AS hfs INNER JOIN (SELECT dbo_UV_HFSLoans_NOSHFD.LoanNumber, Max(dbo_UV_HFSLoans_NOSHFD.FundingDate) AS FundingDate FROM dbo_UV_HFSLoans_NOSHFD GROUP BY dbo_UV_HFSLoans_NOSHFD.LoanNumber)  AS unqhfs ON (hfs.FundingDate = unqhfs.FundingDate) AND (hfs.LoanNumber = unqhfs.LoanNumber)) LEFT JOIN tblMPFLoansAggregateSource AS mpf ON hfs.LoanNumber = mpf.LoanNumber;


------- qryInsertHFSLoansEOD 20141002 Hua
SELECT 	hfs.LoanNumber,
		hfs.LoanInvestmentStatus,
		hfs.ChicagoParticipation,
		hfs.InterestRate,
		...
		Cdbl(0) AS Price
INTO tblHFSLoansEOD
FROM dbo_UV_HFSLoans_NOSHFD AS hfs LEFT JOIN tblMPFLoansAggregateSource mpf ON (hfs.LoanNumber = mpf.LoanNumber)
WHERE hfs.LoanInvestmentStatus not in ("03", "06", "09")
...
-- qryAppendUnhedgedDataTotblMarkToModelLoans
INSERT INTO tblMarkToModelLoans ( AccountClass, AccountType, ScheduleType, PassThruRate, Portfolio, CUSIP, CUSIPOriginal, H2, AggNotional, Mult, AggFactor, [Add Accrued?], PV, Swap, AggWac, AggCoup, AggWam, AggAge, H1, Swam, AggOWAM, [P/O], AggPrice, OAS, Settle, Repo, NxtPmt, PP, PPConst, PPMult, PPSense, Lag, Lock, PPFq, NxtPP, NxtRst1, AggPrepWac, AggPrepCoup, FA, Const1, Const2, Rate, RF, Floor, Cap, PF, PF1, PF2, PC, AB, NxtRst2, LookBackRate, LookBack, WamDate, PPUnits, PPCRShift, RcvCurrEscr, PayCurrEscr, AggBookPrice, AmortMethod, ClientName, [Active?], Agency, [Int Coup Mult], [PP Coup Mult], [Sum Floor], [Sum Cap], [Servicing Model], [Loss Model], [Sched Cap?], Delay, Ballon, IF, Mult1, Mult2, PrepaymentInterestRate )
SELECT AccountClass, AccountType, ScheduleType, PassThruRate, Last(Portfolio) AS Portfolio, Last(CUSIP) AS CUSIP, Last(CUSIPOriginal) AS CUSIPOriginal, Count(LoanNumber) AS H2, Sum(Notional) AS AggNotional, Last(Mult) AS Mult, IIf(Sum([CurrentLoanBalance])=0,Sum([OriginalAmount])/Sum([OriginalAmount]),Sum([CurrentLoanBalance])/Sum([OriginalAmount])) AS AggFactor, Last([Add Accrued?]) AS [Add Accrued?], Last(PV) AS PV, Last(Swap) AS Swap, (Sum([Wac]*[AcctBalance]*[ChicagoParticipation])/Sum([AcctBalance]*[ChicagoParticipation])) AS AggWac, (Sum([Coup]*[AcctBalance]*[ChicagoParticipation])/Sum([AcctBalance]*[ChicagoParticipation])) AS AggCoup, CInt(Sum([Wam]*[AcctBalance]*[ChicagoParticipation])/Sum([AcctBalance]*[ChicagoParticipation])) AS AggWam, (Sum([Age]*[AcctBalance]*[ChicagoParticipation])/Sum([AcctBalance]*[ChicagoParticipation])) AS AggAge, IIf(Val([AccountType])=30 And [AggWam]<340,"Seasoned",IIf(Val([AccountType])=20 And [AggWam]<220,"Seasoned",IIf(Val([AccountType])=15 And [AggWam]<160,"Seasoned","MPF"))) AS H1, Last(Swam) AS Swam, CInt(Sum([OWAM]*[AcctBalance]*[ChicagoParticipation])/Sum([AcctBalance]*[ChicagoParticipation])) AS AggOWAM, IIf([H1]="Seasoned","OAS","Price") AS [P/O], 0 AS AggPrice, 0 AS OAS, Last(Settle) AS Settle, Last(Repo) AS Repo, Last(NxtPmt) AS NxtPmt, "AFT" AS PP, Last(PPConst) AS PPConst, Last(PPMult) AS PPMult, Last(PPSense) AS PPSense, Last(Lag) AS Lag, Last(Lock) AS Lock, Last(PPFq) AS PPFq, Last(NxtPP) AS NxtPP, Last(NxtRst1) AS NxtRst1, (Sum([PrepWac]*[AcctBalance]*[ChicagoParticipation])/Sum([AcctBalance]*[ChicagoParticipation])) AS AggPrepWac, (Sum([PrepCoup]*[AcctBalance]*[ChicagoParticipation])/Sum([AcctBalance]*[ChicagoParticipation])) AS AggPrepCoup, Last(FA) AS FA, Last(Const1) AS Const1, Last(Const2) AS Const2, Last(Rate) AS Rate, Last(RF) AS RF, Last(Floor) AS Floor, Last(Cap) AS Cap, Last(PF) AS PF, Last(PF1) AS PF1, Last(PF2) AS PF2, Last(PC) AS PC, Last(AB) AS AB, Last(NxtRst2) AS NxtRst2, Last(LookBackRate) AS LookBackRate1, Last(LookBack2) AS LookBack2, Last(CStr([qryUnhedgedPalmsDataSource].[WamDate])) AS WamDate, Last(PPUnits) AS PPUnits, Avg(PPCRShift) AS PPCRShift, Last(RcvCurrEscr) AS RcvCurrEscr, Last(PayCurrEscr) AS PayCurrEscr, Sum([CurrentLoanBalance]*[ChicagoParticipation]) AS AggBookPrice, Last(AmortMethod) AS AmortMethod, Last(ClientName) AS ClientName, Last([Active?]) AS [Active?], Last(Agency) AS Agency, Last([Int Coup Mult]) AS [Int Coup Mult], Last([PP Coup Mult]) AS [PP Coup Mult], Last([Sum Floor]) AS [Sum Floor], Last([Sum Cap]) AS [Sum Cap], Last([Servicing Model]) AS [Servicing Model], Last([Loss Model]) AS [Loss Model], Last([Sched Cap?]) AS [Sched Cap?], Last(Delay) AS Delay, Last(Ballon) AS Ballon, Last(IF) AS IF, Last(Mult1) AS Mult1, Last(Mult2) AS Mult2, (Sum([PrepaymentInterestRate]*[AcctBalance]*[ChicagoParticipation])/Sum([AcctBalance]*[ChicagoParticipation])) AS AggPrepaymentInterestRate
FROM qryUnhedgedPalmsDataSource
GROUP BY AccountClass, AccountType, ScheduleType, PassThruRate, OriginationYear
HAVING ((((Sum([Age]*[AcctBalance]*[ChicagoParticipation])/Sum([AcctBalance]*[ChicagoParticipation])))>30))
ORDER BY OriginationYear, AccountType DESC , ScheduleType DESC , (Sum([PrepaymentInterestRate]*[AcctBalance]*[ChicagoParticipation])/Sum([AcctBalance]*[ChicagoParticipation]));
...
-- qryAppendMPFSecuritiesToInstrumentTableP
-- INSERT INTO [Instrument-Palms] ( CUSIP, [Active?], Agency, Delay, Wac, Coup, Balloon, Owam, IF, PF, FA1, Const1, Mult1, Rate1, RF1, Floor1, Cap1, PF1, PC1, AB1, LookBack1, [Int Coup Mult], [PP Coup Mult], [Sum Floor], [Sum Cap], [Servicing Model], [Loss Model], [Sched Cap?], BondCUSIP )
-- SELECT CUSIP, [Active?], Agency, Delay, AggWac, AggCoup, Ballon, AggOWAM, IF, PF, FA, Const1, Mult1, Rate, RF, Floor, Cap, PF1 AS PF1, PC, AB, LookBack, [Int Coup Mult], [PP Coup Mult], [Sum Floor], [Sum Cap], [Servicing Model], [Loss Model], [Sched Cap?], Portfolio
-- FROM tblSecuritiesPalmsSource;
...
---=========== update MPF price (from xls to tbl)
